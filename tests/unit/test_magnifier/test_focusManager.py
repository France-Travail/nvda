...
from _magnifier.utils.types import FocusType
import unittest
...
class FocusTestParam:
	...
	navigatorObjectPos: locationHelper.Point
	systemFocusPos: locationHelper.Point
	mousePos: tuple
	leftPressed: bool
	expectedCoords: locationHelper.Point
...
class TestFocusManager(unittest.TestCase):
	...
	def testFocusManagerCreation(self):
		...
		self.assertEqual(self.focusManager._lastSystemFocusPosition, locationHelper.Point(0, 0))
		self.assertEqual(self.focusManager._lastNavigatorObjectPosition, locationHelper.Point(0, 0))
		self.assertEqual(self.focusManager._lastMousePosition, locationHelper.Point(0, 0))
	...
	def testGetNavigatorObjectPosition(self):
		...
		with patch("_magnifier.utils.focusManager.api.getReviewPosition") as mock_review:
			...
			self.assertEqual(coords, locationHelper.Point(300, 400))
		...
		with patch("_magnifier.utils.focusManager.api.getReviewPosition", return_value=None):
			with patch("_magnifier.utils.focusManager.api.getNavigatorObject") as mock_navigator:
				...
				self.assertEqual(coords, locationHelper.Point(200, 300))
	...
	def testGetSystemFocusPosition(self):
		...
		with patch("_magnifier.utils.focusManager.api.getCaretPosition") as mock_caret:
			...
			self.assertEqual(coords, locationHelper.Point(500, 600))
		...
		with patch("_magnifier.utils.focusManager.api.getCaretPosition", side_effect=RuntimeError):
			with patch("_magnifier.utils.focusManager.api.getFocusObject") as mock_focus:
				...
				self.assertEqual(coords, locationHelper.Point(250, 340))
	...
	def testGetMousePosition(self):
		...
		with patch("_magnifier.utils.focusManager.winUser.getCursorPos", return_value=(123, 456)):
			...
			self.assertEqual(coords, locationHelper.Point(123, 456))

	def testGetCurrentFocusCoordinates(self):
		"""All priority scenarios for focus coordinates."""
		subTestParams = [
			FocusTestParam(
				navigatorObjectPos=locationHelper.Point(0, 0),
				systemFocusPos=locationHelper.Point(0, 0),
				mousePos=locationHelper.Point(0, 0),
				leftPressed=True,
				expectedCoords=locationHelper.Point(0, 0),
			...
			FocusTestParam(
				navigatorObjectPos=locationHelper.Point(0, 0),
				systemFocusPos=locationHelper.Point(0, 0),
				mousePos=locationHelper.Point(10, 10),
				leftPressed=False,
				expectedCoords=locationHelper.Point(10, 10),
				expectedFocus=FocusType.MOUSE,
				description="Mouse moving (not dragging)",
			),
			FocusTestParam(
				navigatorObjectPos=locationHelper.Point(0, 0),
				systemFocusPos=locationHelper.Point(15, 15),
				mousePos=locationHelper.Point(0, 0),
				leftPressed=False,
				expectedCoords=locationHelper.Point(15, 15),
				expectedFocus=FocusType.SYSTEM_FOCUS,
				description="System focus changed alone (navigator did not move)",
			),
			FocusTestParam(
				navigatorObjectPos=locationHelper.Point(20, 20),
				systemFocusPos=locationHelper.Point(0, 0),
				mousePos=locationHelper.Point(0, 0),
				leftPressed=False,
				expectedCoords=locationHelper.Point(20, 20),
				expectedFocus=FocusType.NAVIGATOR,
				description="Navigator object changed (NumPad navigation)",
			),
			FocusTestParam(
				navigatorObjectPos=locationHelper.Point(30, 30),
				systemFocusPos=locationHelper.Point(15, 15),
				mousePos=locationHelper.Point(0, 0),
				leftPressed=False,
				expectedCoords=locationHelper.Point(30, 30),
				expectedFocus=FocusType.NAVIGATOR,
			...
			FocusTestParam(
				navigatorObjectPos=locationHelper.Point(10, 10),
				systemFocusPos=locationHelper.Point(0, 0),
				mousePos=locationHelper.Point(20, 20),
				leftPressed=False,
				expectedCoords=locationHelper.Point(20, 20),
				expectedFocus=FocusType.MOUSE,
		...
		for param in subTestParams:
			with self.subTest(description=param.description):
				...
				self.focusManager._lastNavigatorObjectPosition = locationHelper.Point(0, 0)
				self.focusManager._lastSystemFocusPosition = locationHelper.Point(0, 0)
				self.focusManager._lastMousePosition = locationHelper.Point(0, 0)
